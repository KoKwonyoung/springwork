...
<script type="text/javascript" >
	...
	function userSelect() {
		...
		$('body').on('click','#btnSelect',function(){
			...
			//특정 사용자 조회
			$.ajax({
				url:'users/'+userId, //여기는 컨트롤러 // 'users/'는 컨트롤러에 있는 value="/users/
				//data : {userid:userId}, // 'userid=asdf' 여기는 VO로 받는
				//type:'GET',
				//contentType:'application/json;charset=utf-8',
				dataType:'json',
				error:function(xhr,status,msg){
	...
	function userUpdate() {
		...
		$('#btnUpdate').on('click',function(){
			...
			$.ajax({ 
			    ...
			    contentType: 'application/json', //내가 보내는 request가 JSON으로 보내겠다는것
